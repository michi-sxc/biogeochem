...
# "Large Lakes" and "Flooded Grasslands" have 0 variance. 
# Exclude from trends, keep for total burial area.
...
cat("\nNOTE: Zero variance detected in:", paste(constant_biomes, collapse = ", "), "\n")
cat("-> Action: Excluded from trend stats; included in total burial sums.\n\n")
...
globocarb <- globocarb %>%
  ...
  mutate(
    ...
    # Steffen et al. (2015) Anthropocene marker
    Period = factor(
      ifelse(Decade < 1950, "Pre-1950", "Post-1950"),
      levels = c("Pre-1950", "Post-1950")
    ),
    
    # Tg C yr-1 = g/m2/yr * km2 * 1e6 / 1e12
...
# Reliable subset (excl. 2 constant-OCAR biomes)
...
# Group definitions for later analysis
...
#           right-skewed, dominated by low-burial
...
# --- 5.2 Boxplot by Biome + Temporal Jitter ----------------------------------
# Palette: Blue (1900) -> Orange (2000)
# vertical jitter has no meaning, just visualisation purpose
...
fig2 <- ggplot(globocarb,
  ...
  # Highlight zero-variance biomes
  annotate("rect",
...
# Note: Boreal dominates total burial due to Area, despite low OCAR.
...
# Decadal sums: lakes, reservoirs + combined
...
# Cumulative Calc (Pg C)
cat("\n--- Cumulative C Burial: Trapezoidal Integration (1900-2000) ---\n")
cat("  Each decade value represents a rate (Tg C/yr) for a 10-year window.\n")
cat("  Cumulative burial = sum of (rate * 10 years), converted to Pg C.\n\n")

cumulative_lake_PgC <- sum(decade_totals$Lake_Burial_Tg) * 10 / 1000
cumulative_total_PgC <- sum(decade_totals$Total_Burial_Tg) * 10 / 1000

cat("  Cumulative lake burial (1900-2000):",
    round(cumulative_lake_PgC, 2), "Pg C\n")
cat("  Cumulative lake + reservoir burial (1900-2000):",
    round(cumulative_total_PgC, 2), "Pg C\n")
cat("  NOTE: This assumes each decadal rate is constant over its 10-year window.\n")

# Uncertainty (2000)
cat("\n--- Uncertainty Propagation for Total Burial (2000) ---\n")
cat("  Approach: propagate OCAR standard deviation through burial formula.\n")
cat("  Burial = OCAR * Area * 1e6 / 1e12; uncertainty from SD(OCAR) per biome.\n\n")

burial_uncertainty <- globocarb %>%
  filter(Decade_num == 2000) %>%
  group_by(Biome) %>%
  summarize(
    OCAR_val  = mean(OCAR),
    OCAR_sd   = sd(globocarb$OCAR[globocarb$Biome == unique(Biome)]),
    Area      = mean(Lake_Area),
    Burial    = (OCAR_val * Area * 1e6) / 1e12,
    Burial_sd = (OCAR_sd  * Area * 1e6) / 1e12,
    .groups   = "drop"
  )

total_burial_2000    <- sum(burial_uncertainty$Burial)
total_burial_2000_sd <- sqrt(sum(burial_uncertainty$Burial_sd^2))  # quadrature

cat("  Total lake burial in 2000:", round(total_burial_2000, 1), "Tg C/yr\n")
cat("  Combined uncertainty (1 SD, quadrature):",
    round(total_burial_2000_sd, 1), "Tg C/yr\n")
cat("  Reported as:", round(total_burial_2000, 1), "\u00b1",
    round(total_burial_2000_sd, 1), "Tg C/yr\n")
cat("  NOTE: This uses temporal SD as a proxy for rate uncertainty.\n")
cat("         A conservative approach; true uncertainty also includes area errors.\n")
...
biome_means <- globocarb %>%
  ...
  summarize(
    Mean_OCAR   = mean(OCAR),
    Lake_Area   = first(Lake_Area), #assuming constant
...
#  Test hypothesis that tropical biomes show faster OCAR increases
...
# Fit linear models for annotation
...
# small check of model assumptions (normality, homoscedasticity).
...
# --- 6.1b Robustness: GLS with AR(1) errors ----------------------------------
cat("\n--- 6.1b Robustness Check: GLS with AR(1) autocorrelation structure ---\n")
cat("  The pooled OLS above treats biome-decade observations as independent.\n")
cat("  Within each biome, consecutive decades are temporally autocorrelated.\n")
cat("  GLS with AR(1) errors corrects for this, giving conservative SE estimates.\n\n")

library(nlme)
gls_ar1 <- gls(OCAR ~ Decade_num,
               data      = gc_reliable,
               correlation = corAR1(form = ~ Decade_num | Biome),
               method    = "ML")
print(summary(gls_ar1))

cat("\n  Comparison:\n")
cat("  OLS slope:     ", round(coef(model_global)[2], 4),
    "(SE =", round(summary(model_global)$coefficients[2,2], 4), ")\n")
cat("  GLS AR(1) slope:", round(coef(gls_ar1)[2], 4),
    "(SE =", round(summary(gls_ar1)$tTable[2,2], 4), ")\n")
cat("  If slopes and significance agree: OLS result is robust to autocorrelation.\n")
...
biome_ttest <- gc_reliable %>%
  ...
  mutate(
    p_bonferroni  = p_raw * n_distinct(gc_reliable$Biome),
...
# --- 6.5a Tropical vs. Boreal: Interaction Models -----------------------------
...
#          independent-->  problematic because biomes within region are not
...
# Model B: Mixed-effects interaction with Biome as random intercepts:
#          Accounts that tropical biomes (Mangroves, Tropical
#          Forest, Tropical Grasslands) have different baselines but share
#          a regional grouping--> random intercept absorbs region-level variance
...
# --- 6.5b Sensitivity: Tropical slope WITHOUT Mangroves ----------------------
cat("\n--- 6.5b Sensitivity: H2 re-tested excluding Mangroves ---\n")
cat("  Mangroves are coastal/tidal systems, not strictly freshwater lakes.\n")
cat("  Testing whether the tropical-boreal divergence holds without them.\n\n")

tropical_no_mang <- c("Tropical Moist and Dry Forest", "Tropical Grasslands")

regional_nomang <- gc_reliable %>%
  filter(Biome %in% c(tropical_no_mang, boreal_biomes)) %>%
  mutate(Region = factor(
    ifelse(Biome %in% tropical_no_mang, "Tropical", "Boreal/Tundra"),
    levels = c("Boreal/Tundra", "Tropical")
  ))

slope_trop_nomang <- coef(lm(OCAR ~ Decade_num,
                             data = filter(regional_nomang, Region == "Tropical")))[2]
slope_bor_nomang  <- coef(lm(OCAR ~ Decade_num,
                             data = filter(regional_nomang, Region == "Boreal/Tundra")))[2]

cat("  Tropical slope (excl. Mangroves):", round(slope_trop_nomang, 3), "/decade\n")
cat("  Boreal slope:", round(slope_bor_nomang, 3), "/decade\n")
cat("  Ratio:", round(slope_trop_nomang / slope_bor_nomang, 1), "x\n\n")

interaction_nomang <- lm(OCAR ~ Decade_num * Region, data = regional_nomang)
cat("  Interaction model (fixed effects, excl. Mangroves):\n")
print(summary(interaction_nomang))

interaction_mixed_nomang <- lmer(OCAR ~ Decade_num * Region + (1 | Biome),
                                 data = regional_nomang)
cat("  Mixed-effects interaction (excl. Mangroves):\n")
print(summary(interaction_mixed_nomang))
...
# b) Detrended correlation: remove linear time trend from both variables,
#    then correlate the residuals--> association by temporal trends if correlation vanishes
...
cat("  This is the most appropriate model for this dataset\n")
...
# Center decade for numerical stability and interpretable intercept #UPDATE: can be centered, but is scaled now
...
# --- 6.9 Sensitivity Analysis (suspect biomes) ---------------
...
if (cor_detrended$p.value < 0.05) {
  cat("7. Reservoir-OCAR detrended correlation is significant (rho =",
